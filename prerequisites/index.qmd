...
1. Open the terminal you verified in Step 2 (Windows Terminal recommended)
2. Install Claude Code using npm (Node.js must be installed first):
   ```cmd
   npm install -g @anthropic-ai/claude-code
   ```
...
::: {.callout-note}
If you encounter issues on Windows, you may need to use Windows Subsystem for Linux (WSL). See the troubleshooting section for WSL setup instructions.
:::
:::
...
- Windows users: If the standard installation fails, you may need to use WSL:
  1. Install WSL: `wsl --install` (in PowerShell as Administrator)
  2. Restart your computer
  3. Open WSL (Ubuntu) from Start menu
