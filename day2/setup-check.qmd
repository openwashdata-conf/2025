...
# Navigate to your working directory
cd C:\Users\%USERNAME%\Documents\gitrepos

# Start Claude Code through WSL
wsl claude
```

::: {.callout-tip}
## Windows Pro Tip
Create a batch file `claude.bat` for easy launching:
```batch
@echo off
cd C:\Users\%USERNAME%\Documents
wsl claude %*
```
Save this in a directory that's in your PATH for quick access from anywhere.
:::
:::

Once Claude Code starts:
- Confirm Max plan is active
- Test basic functionality by asking Claude a simple question
...
- Verify subscription status at [claude.ai](https://claude.ai)

## Windows
- Ensure WSL is running: `wsl --status`
- Inside WSL, check installation: `which claude`
- Verify Node.js in WSL: `node --version`
- Try the direct command: `wsl /usr/local/bin/claude`
- Check the [Windows Installation Guide](https://github.com/VonHoltenCodes/CLAUDECODE_COMPLETE_WINDOWS_INSTALL_GUIDE/blob/main/docs/INSTALLATION.md) for troubleshooting
:::
